...
public class Gen {
	...
	/**
	 * method for invoking as a stand alone program
	 * 
	 * @param args
	 */
	public static void main(String[] args) {
		...
		SudokuBoard.PrintPossible(board);
	}
	
	/**
	 * Returns a randomly generated SudokuBoard
	 * 
	 * @param size square number to be used for the total board dimensions
	 * 
	 * @return randomized board
	 */
	public static SudokuBoard GetBoard(int size) {
	 ...
	 * 
	 * @param board the SudokuBoard to be filled
	 */
	private static void FirstRow(SudokuBoard board) {
		
		//array list with remaining values to be used
		ArrayList<Integer> values = new ArrayList<Integer>();
		
		//cursor to fill in the top row (row defaults to 0)
		Location cursor = new Location();
	...
	private static SudokuBoard FillBoard(SudokuBoard board) {
		...
		while (!guesses.isEmpty() && testBoard == null) {
			...
			testBoard = (SudokuBoard) board.clone();
	 ...
	 * 
	 * @param board SudokuBoard to be probed
	 * 
	 * @return possible solutions for a random square with lowest number of possibilities
	 */
	private static ArrayList<Location> FindTestGuesses(SudokuBoard board) {
		
		//declare space in memory for the return list
		ArrayList<Location> return_list = new ArrayList<Location>();
		
		//set lowest to max value
		int lowest = size;
		
		//local address for a square on the SudokuBoard's possibilities
		boolean[] possible;
		...
		for(int row = 0; row < size; row++)
			for(int column = 0; column < size; column++) {
				...
				//add location to the list if it's count is equal to the lowest
				if (count == lowest)
					return_list.add(new Location(row, column, 0));
			}
		...
		for(int cursor = 0; cursor < size; cursor++)
			if (possible[cursor])
				return_list.add(new Location(row, column, cursor+1));
